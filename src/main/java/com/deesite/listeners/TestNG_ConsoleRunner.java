...
public class TestNG_ConsoleRunner extends TestListenerAdapter {
    ...
    public void onStart(ITestContext testContext) {
        try {
            log("\nSuite Start Date: " +
                    new SimpleDateFormat("MM.dd.yyyy.HH.mm.ss").format(new Date()) + ".log");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void onConfigurationFailure(ITestResult tr) {
        ...
        log(tr.getEndMillis(), "END CONFIG -> "
                + tr.getInstanceName() + "."
                + tr.getName());
        log("\n---\n");
            ...
            if (tr.getParameters().length > 0) {
                ...
                for (int iCount = 0; iCount < iLength; iCount++) {
                    if (iCount == 0) {
                        ...
                        message = message
                                + ", "
                                + tr.getParameters()[iCount].toString();
                    }
                }
            }
        } catch (Exception e) {
